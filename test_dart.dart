...
class BrgyRainThreatUtil {
  static const _geoRiskUrl = "https://portal.georisk.gov.ph/arcgis/rest/services/PSA/Barangay/MapServer/4/query?where=city_name%20LIKE%20%27%25Laoag%25%27&outFields=brgy_name,city_name,prov_name,brgy_code&returnGeometry=true&outSR=4326&f=geojson";
  ...
  static List<List<double>> _flattenPairs(dynamic coords, [List<List<double>>? pairs]) {
    ...
    if (coords is List) {
      if (coords.length >= 2 && coords.every((value) => value is num)) {
        pairs.add([(coords[0] as num).toDouble(), (coords[1] as num).toDouble()]);
        return pairs;
      }
      for (var child in coords) { _flattenPairs(child, pairs); }
    }
    return pairs;
  }
  ...
  static Map<String, double>? _getCenter(Map<String, dynamic> feature) {
    ...
    for (var pair in pairs) {
      minLng = min(minLng, pair[0]); maxLng = max(maxLng, pair[0]);
      minLat = min(minLat, pair[1]); maxLat = max(maxLat, pair[1]);
    }
  ...
  static Future<Map<String, dynamic>> fetchAndEnrich() async {
    ...
    if (geoRiskRes.statusCode != 200) throw Exception("GeoRiskPH fetch failed with status \${geoRiskRes.statusCode}");
    final geoJson = jsonDecode(geoRiskRes.body) as Map<String, dynamic>;
    final featuresList = geoJson['features'];
    if (featuresList == null || featuresList is! List) throw Exception("GeoRiskPH returned invalid features list");
    ...
    for (int i = 0; i < featuresList.length; i++) {
      ...
      if (center != null) validRows.add({'feature': feature, 'center': center});
    }
    for (int start = 0; start < validRows.length; start += _batchSize) {
      ...
      final queryParams = {
        'latitude': lats, 'longitude': lngs, 'hourly': 'precipitation', 'current': 'precipitation,weather_code',
        'forecast_hours': '24', 'timezone': 'Asia/Manila',
      };
      ...
      try {
        ...
        if (meteoRes.statusCode == 200) {
          ...
          for (int i = 0; i < batch.length; i++) {
            ...
            final row = batch[i]; final forecast = forecasts[i];
        ...
        } else { debugPrint("Open-Meteo batch failed with status \${meteoRes.statusCode}"); }
      } catch (e) { debugPrint("Open-Meteo fetch error: \$e"); }
    }
...
void main() async {
  try {
    ...
    print("SUCCESS! Enriched \${res['features'].length} features.");
    print("Sample feature 0: \${res['features'][0]['properties']['rain_level']}");
  } catch(e) {
    print("ERROR: \$e");
  }
}
